...
BW_CLIENTID=$(echo "$secret_data" | grep '"name": "client_id"' -A 1 | grep value | sed 's/.*"\(.*\)".*/\1/')
BW_CLIENTSECRET=$(echo "$secret_data" | grep '"name": "client_secret"' -A 1 | grep value | sed 's/.*"\(.*\)".*/\1/')
